...
def set_button_irq(timer):
    button.irq(handler=debounce_button, trigger=Pin.IRQ_FALLING)

def debounce_button(pin):
    pin.irq(handler=None)
    global timer, fm_radio
    settings = fm_radio.GetSettings()
    current_vol = settings[1]
    new_vol = 1 if current_vol >= 15 else current_vol + 1
    print(f"volume: {new_vol}")
    fm_radio.SetVolume(new_vol)
    fm_radio.ProgramRadio()
    timer.init(mode=Timer.ONE_SHOT, period=175, callback=set_button_irq)
...
class Radio:
    def __init__( self, NewFrequency, NewVolume, NewMute ):
        ...
        self.Mute = False
        self.MaxTries = 10 # how many times it'll attempt communication before giving up
    ...
    def ProgramRadio(self):
        ...
        success = False
        counter = 0
        while not success and counter < self.MaxTries:
            try:
...
# 101.9 is CFUV, the campus station
fm_radio = Radio( 101.9, 1, False )
